    ...
    *,
    ...
    for i, pair in enumerate(items):
        ...
        if not isinstance(vector, DenseVector):
            raise TypeError(
                f"items[{i}][0] must be a DenseVector"
...
def build_batch_search_request(
    *,
    ...
    if not isinstance(queries, (list,tuple)):
        raise TypeError("Queries must be a list of (DenseVector, Similarity, Limit (int)) tuples")
    ...
    for i, trio in enumerate(queries):
        if not isinstance(trio, (list,tuple)) or len(trio)!=3:
            raise TypeError(f"queries[{i}] must be a tuple of (DenseVector, Similarity, Limit(int))")
        ...
        requests.append(vector_db_pb2.SearchRequest(
                query_vector=vector.to_proto(),
                similarity=similarity.to_proto(),
                limit=limit,
                ef=ef,
